...
class TestUserRepository:
    ...
    async def test_get_by_yandex_id(self, mock_db, mock_user):
        """Test getting user by Yandex ID"""
        repo = UserRepository(mock_db)
        yandex_id = "yandex123"
    ...
    async def test_create_user_yandex(self, mock_db):
        """Test creating a user from Yandex data"""
        repo = UserRepository(mock_db)
        yandex_data = {
            "id": "yandex123",
            "default_email": "yandex@example.com",
            "first_name": "Yandex User"
        }
    ...
    async def test_create_user_yandex_alternate_email_field(self, mock_db):
        """Test creating a user from Yandex data with alternate email field"""
        repo = UserRepository(mock_db)
        yandex_data = {
            "id": "yandex123",
    ...
    async def test_get_all_admins(self, mock_db, mock_user):
        ...
        result_mock = MagicMock()
        result_mock.scalars.return_value.all.return_value = admin_users
        mock_db.execute.return_value = result_mock
