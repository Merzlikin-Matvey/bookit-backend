...
class TestReservationsAPI:
    ...
    def _ensure_seats_exist(self):
        """Проверяет, существуют ли места, и создает их при необходимости"""
        try:
            ...
            check_response = admin_session.get(SEATS_URL, params={
    ...
    def login_as_admin(self):
        """Авторизация как админ, данные берутся из data.py"""
        admin_login = {
    ...
    def login_as_user1(self):
        """Авторизация как пользователь 1, данные берутся из data.py"""
        user_login = {
    ...
    def login_as_user2(self):
        """Авторизация как пользователь 2, данные берутся из data.py"""
        user_login = {
    ...
    def test_user_create_reservation(self):
        """Тест создания бронирования пользователем"""
    ...
    def _cancel_all_user_reservations(self, session):
        """Вспомогательный метод для отмены всех бронирований пользователя"""
        response = session.get(RESERVATIONS_URL)
        if response.status_code == 200:
            reservations = response.json()
            for reservation in reservations:
                cancel_response = session.patch(
    ...
    def test_user_cannot_create_multiple_active_reservations(self):
        """Проверка, что пользователь не может создать несколько активных бронирований."""
        user_session = self.login_as_user1()
        self._cancel_all_user_reservations(user_session)
    ...
    def test_cancel_and_create_new_reservation(self):
        """Тест отмены бронирования и создания нового"""
        user_session = self.login_as_user1()
    ...
    def test_admin_can_create_reservation_for_user(self):
        """Тест, что админ может создать бронирование для пользователя"""
    ...
    def test_admin_can_create_after_canceling(self):
        """Тест, что админ может создать бронирование после отмены предыдущего"""
        admin_session = self.login_as_admin()
        ...
        cancel_response = admin_session.patch(
            f"{BASE_URL}/admin/reservation/{reservation_id}", 
            json={"status": "closed"}
        )
        assert cancel_response.status_code == 200, f"Не удалось отменить бронирование: {cancel_response.text}"
    ...
    def test_different_users_can_have_active_reservations(self):
        """Тест, что разные пользователи могут иметь активные бронирования"""
    ...
    def test_update_reservation(self):
        """Тест обновления бронирования (изменение времени)"""
        user_session = self.login_as_user1()
    ...
    def test_user_cannot_modify_others_reservation(self):
        """Тест, что пользователь не может изменять чужие бронирования"""
        user1_session = self.login_as_user1()
        self._cancel_all_user_reservations(user1_session)
    ...
    def test_admin_can_modify_any_reservation(self):
        """Тест, что админ может изменить любое бронирование"""
        user1_session = self.login_as_user1()
        self._cancel_all_user_reservations(user1_session)
    ...
    def test_admin_can_delete_reservation(self):
        """Тест, что админ может удалить бронирование"""
        user1_session = self.login_as_user1()
        self._cancel_all_user_reservations(user1_session)
    ...
    def test_get_active_reservation(self):
        """Тест получения активного бронирования"""
        user_session = self.login_as_user1()
    ...
    def test_admin_get_all_reservations(self):
        """Тест получения всех бронирований в системе админом"""
        admin_session = self.login_as_admin()
